...
class TimeAttackGameViewController: UIViewController {
    ...
    // Time remaining
    @IBOutlet weak var lblTextRemain: UILabel!
    private var defaultTime: Int = 20
    private var timeRemain: Int = 0
    private var timerTimeRemain: Timer!
    ...
    override func viewDidLoad() {
        ...
        // Set time remaining
        timeRemain = defaultTime
        lblTextRemain.text = String(format: "%02d:%02d", arguments: [defaultTime / 60, defaultTime % 60])
        timerTimeRemain = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { timer in
            self.timeRemain -= 1
            self.lblTextRemain.text = String(format: "%02d:%02d", arguments: [self.timeRemain / 60, self.timeRemain % 60])
            self.lblTotalTime.frame.size.width = CGFloat(298 * (Float(self.timeRemain) / Float(self.defaultTime)))
        }
    ...
    @IBAction func timeBtn(_ sender: Any) {
        if timeRemain + 15 > defaultTime {
            defaultTime = timeRemain + 15
            timeRemain = defaultTime
        } else {
            timeRemain += 15
        }
